...
export function DailyRoutineClient({ initialLeads, initialReport }: any) {
  ...
  // Math Auto-Fills (Módulo 6 Matrix)
  const contactedOver48h = initialLeads.filter((l: any) => l.status === 'contacted' && ((Date.now() - new Date(l.updated_at).getTime()) / (1000 * 60 * 60)) > 48).length;
  ...
  const syncMetrics = async () => {
    ...
    try {
      ...
      toast.error("Failed executing sync protocol.");
    } finally {
  ...
  const Task = ({ id, label, highlight }: { id: string, label: string, highlight?: string }) => (
    <div 
      ...
      className="flex items-start gap-4 p-4 rounded-xl cursor-pointer hover:bg-zinc-900/40 border border-transparent hover:border-zinc-800 transition-colors group"
    >
      ...
      <div className={`text-sm font-bold leading-relaxed ${tasks[id] ? 'text-zinc-600 line-through' : 'text-zinc-300'}`}>
         {label}
         {highlight && <span className="ml-2 bg-orange-950/40 text-orange-500 border border-orange-900/50 px-2 py-0.5 rounded text-[10px] uppercase font-black uppercase tracking-widest">{highlight}</span>}
      </div>
    </div>
  );
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-10">
      ...
      <div className="lg:col-span-7 space-y-6">
         ...
         <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="flex flex-col">
               <Task id="prep_1" label="Revisar leads en 'Contactado' con +48h sin respuesta → mover o descartar" highlight={contactedOver48h > 0 ? `${contactedOver48h} Stalled Targets` : undefined} />
               <Task id="prep_2" label="Revisar leads en 'Conversación activa' → anotar pendientes" highlight={`${activeConversations} Active Convs`} />
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
         <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 shadow-xl relative overflow-hidden sticky top-6">
            ...
            <div className="space-y-6">
               <div className="flex items-center justify-between">
                  ...
                  <input 
                    ...
                    className="bg-black border border-zinc-800 w-20 text-center px-2 py-1.5 rounded-lg focus:outline-none focus:border-zinc-500 font-black text-white"
                  />
               </div>
               ...
               <div className="flex items-center justify-between">
                  ...
                  <input 
                    ...
                    className="bg-black border border-zinc-800 w-20 text-center px-2 py-1.5 rounded-lg focus:outline-none focus:border-cyan-500 font-black text-cyan-400"
                  />
               </div>
