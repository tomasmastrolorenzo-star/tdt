            ...
            <div key={lead.id} className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 shadow-2xl relative overflow-hidden flex flex-col justify-between transition-opacity" style={{ opacity: isProcessing ? 0.5 : 1 }}>
               <div>
                 <div className="flex items-start justify-between mb-5">
                    <h3 className="text-lg font-black text-white truncate max-w-[70%]">@{lead.instagram_username}</h3>
                    <span className="text-[9px] uppercase font-black tracking-[0.2em] px-2 py-1 rounded bg-black border border-zinc-800 text-zinc-400">
               ...
               <div className="flex flex-col gap-2 mt-auto">
                 <button 
                   ...
                   className="w-full bg-white text-black font-black uppercase tracking-[0.2em] text-[10px] py-4 rounded-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(255,255,255,0.1)]"
                 >
                   <MessageCircle className="w-4 h-4" /> Ya le escribí (+2)
                 </button>
                 <div className="flex gap-2">
                   <button 
                     disabled={isProcessing} onClick={() => handleAction(lead.id, 'defer')}
                     className="flex-1 bg-zinc-900 border border-zinc-800 text-zinc-300 font-black uppercase tracking-widest text-[9px] py-3 rounded-xl hover:bg-zinc-800 hover:text-white transition-colors flex items-center justify-center gap-1.5"
                   >
                     <FastForward className="w-3.5 h-3.5" /> Mañana
                   </button>
                   <button 
