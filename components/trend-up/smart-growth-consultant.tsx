...
// UPDATED: Fixed z-index and positioning to prevent overlap
const FinalBlackScreen = ({ text, subtext }: { text: string | null | undefined, subtext?: string }) => {
    return (
        <div className="fixed inset-0 z-[100] min-h-screen bg-black flex flex-col items-center justify-center cursor-none select-none p-8 text-center text-white overflow-hidden">
...
export default function SmartGrowthConsultant({ initialHandle, initialIntent, initialLang }: { initialHandle?: string, initialIntent?: IntentDeclaration, initialLang?: 'EN' | 'ES' | 'PT' }) {
    ...
    if (state === OperationalState.BLACK_HOLE) {
        return (
            // UPDATED: Fixed z-index for Full Screen Overlay
